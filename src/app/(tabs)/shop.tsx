          ...
          <View style={styles.grid}>
            {filtered.map((product, i) => (
              <FadeIn key={product.id} delay={(i % 2) * 60} style={styles.cell}>
                <Pressable onPress={() => router.push(`/product/${product.id}` as any)} style={{ flex: 1 }}>
                  <Image source={product.image} style={styles.image} resizeMode="cover" />
                  <Text style={styles.name} numberOfLines={1}>
                    {product.name}
                  </Text>
                  <Text style={styles.price}>
                    {product.quoteOnly ? 'Request quote' : formatNaira(product.price ?? 0)}
                  </Text>
                </Pressable>
              </FadeIn>
            ))}
  ...
  return useMemo(
    () =>
      StyleSheet.create({
        ...
        name: { fontFamily: fonts.medium, fontSize: 16, color: colors.text, marginTop: sp.x2_ },
