    ...
    <ScrollView
      ...
      <Container style={{ marginTop: sp.x4 }}>
        <FadeIn>
          ...
          <View style={{ flexDirection: 'row', gap: 8, marginTop: sp.x2_ }}>
                ...
                <View style={{ flexDirection: 'row' }}>
                  <View style={{ alignItems: 'center', width: 32 }}>
                    <View
                      style={[
                        styles.dotOuter,
                        completed && { backgroundColor: colors.lime, borderColor: colors.lime },
                        current && { borderColor: colors.lime },
                      ]}>
                    ...
                    {!isLast && (
                      <View
                        style={[styles.line, completed && { backgroundColor: colors.lime }]}
                      />
                    )}
                  </View>
                  <View style={{ paddingBottom: isLast ? 0 : 40, paddingTop: 8, flex: 1 }}>
                    <Text style={[styles.stepLabel, (completed || current) && { color: colors.text }]}>
                      {step}
                    </Text>
                    {current && (
                      <Text style={{ fontFamily: fonts.regular, fontSize: 13, color: colors.isDark ? colors.lime : '#5E8A0D', marginTop: 3 }}>
                        In progress
                      </Text>
                    )}
                  </View>
        ...
        <FadeIn delay={200}>
          <View style={styles.etaCard}>
            <Ionicons name="time-outline" size={18} color={colors.isDark ? colors.lime : '#5E8A0D'} />
...
function useStyles(colors: Palette) {
  return useMemo(
    () =>
      StyleSheet.create({
        orderId: { fontFamily: fonts.bold, fontSize: 40, letterSpacing: -1.2, color: colors.text },
        product: { fontFamily: fonts.medium, fontSize: 17, color: colors.subtext, marginTop: sp.x1 },
